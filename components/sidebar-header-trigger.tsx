...
export function SidebarHeaderTrigger() {
  ...
  return (
    <Button
      ...
      variant="outline"
      size="sm"
      className="h-9 shrink-0 gap-2 border-primary/40 bg-background px-2.5 shadow-sm hover:bg-muted hover:border-primary/60 sm:px-3"
      onClick={handleClick}
      aria-label="Open browse menu — shop categories and collections"
    >
      <PanelLeftIcon className="h-4 w-4 text-primary" aria-hidden />
      <span className="caps text-[11px] font-semibold tracking-wide text-foreground hidden sm:inline">
        Browse more
      </span>
    </Button>
  )
